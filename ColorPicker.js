...
define([],function(){
	...
	*/
	function colorPicker(params,e,mode,size,allowResize,allowClose,allowDrag,expColor,expHEX,offsetX,offsetY,orientation,parentObj,parentXY,color,difPad,rSpeed,cookieLife,docBody){
		// attribute definitions
		var	cP = params || {}, // for now to make code shorter
		onChange = cP.onChange || function(){},
		...
		// private vars
		cP = null,//colorPicker.cP, // collection of parts -> recycling var
		...
		CP=(!cP)?function(obj) { // building the colorPicker / setting up events and methodes
			...
			cP.CTRTop = (cP.CTRTop = new RegExp("\.CTRTop\s*{(.*?)}").exec(CSSTxt))? cP.CTRTop[1]: '';
			cP.cPCD1  = (cP.cPCD1  =  new RegExp("\.cPCD1\s*{(.*?)}").exec(CSSTxt)) ? cP.cPCD1[1] : '';
			cP.cPCD2  = (cP.cPCD2  =  new RegExp("\.cPCD2\s*{(.*?)}").exec(CSSTxt)) ? cP.cPCD2[1] : '';
		...
		doRender = function(xy,yz) { // function for pure rendering. No rendering elsewhere!!
			...
			if (expColor) cP.cObjs.backgroundColor = 'rgb('+CP.rgbRND+')';if (expHEX) cP.cObj.value = CP.valPrefix+CP.hex;onChange(CP.valPrefix+CP.hex);
		...
		X2RGB = function (hex) { // accepts array(r,g,b(,a)), 'rgb(r,g,b)', 'rgba(r,g,b(,a))', #0 - #123AEFxyz, 0 - 123AEFxyz, #2af, 2af
			hex = (hex+'').replace(/[(^rgba\()]*[^a-fA-F0-9,]*/g,'').split(',');
			if (hex.length == 4) hex.pop();
